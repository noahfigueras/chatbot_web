...
const featureColors = [
  ...
  "from-green-400 to-neon-cyan",
  "from-amber-400 to-neon-pink",
  "from-neon-cyan to-neon-purple",
];

function FeatureCard({ icon, title, desc, color, index }: { icon: React.ReactNode; title: string; desc: string; color: string; index: number }) {
  ...
  return (
    <div ref={ref} className="scroll-reveal" style={{ transitionDelay: `${index * 100}ms` }}>
      <div className="glass-card rounded-xl p-6 sm:p-8 h-full hover:shadow-lg hover:shadow-neon-cyan/5 transition-all duration-500 group cursor-default">
        <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${color} p-0.5 mb-5 group-hover:scale-110 transition-transform duration-300`}>
...
export default function Features() {
  ...
  const features = [
    { title: t("items.knowledge.name"), desc: t("items.knowledge.desc") },
    { title: t("items.multiChannel.name"), desc: t("items.multiChannel.desc") },
    { title: t("items.customPrompt.name"), desc: t("items.customPrompt.desc") },
    { title: t("items.analytics.name"), desc: t("items.analytics.desc") },
  ];
